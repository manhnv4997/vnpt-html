$(document).ready(function(){
    ...
    if(n.nodeName == 'P' && $(n).hasClass('exp_area_vote')) {
        var id = $(n).find('.vote_id').text();
        $.ajax({
    		type: "GET",
    		url: '/?mod=vote&act=getVote&id='+id,
    		dataType: "html",
    ...
    $('.btn_one_vote').click(function(){
        ...
        var title = $(this).text();
        ed.execCommand('mceInsertContent', false, '<p class="expNoEdit exp_area_vote widget_vote"><span class="vote_id" style="display:none">'+id+'</span>'+title+'</p>');
        ed.windowManager.close();
        return false;
    });
